...
namespace vtb.TemplatesService.Api.Tests.IntegrationTests
{
    ...
    public class TemplatesEndpointTests_UpdateTemplateVersion : TemplatesEndpointTests
    {
        ...
        [Test]
        public async Task Will_Update_TemplateVersion_And_set_IsActive_To_False()
        {
            Authorize(Guid.NewGuid(), Tenant1Id, Array.Empty<string>(), new[] { Permissions.ManageTemplates });

            await _client.UpdateTemplateVersion(_templateId, _templateVersionId, _request);

            var templateVersion = await _client.GetTemplateVersionDetails(_templateId, _templateVersionId);
            templateVersion.TemplateVersionId.Should().Be(_templateVersionId);
            templateVersion.Version.Should().Be(0);
            templateVersion.Content.Should().BeNullOrEmpty();
        }
    }
}